...
#JAVA_OPTS="$JAVA_OPTS -javaagent:/home/jrxany/batch_node/skywalking-agent/skywalking-agent.jar -Dskywalking.agent.service_name=调度执行节点_master_node -Dskywalking.collector.backend_service=172.16.101.29:11800 "
...
startup(){
    ...
    if [ $TPID -ne 0 ]; then
        ...
        nohup java $JAVA_OPTS -classpath $CLASSPATH $APPLICATION_MAIN --spring.profiles.active=dev > $LOG_DIR/nohup.log &
        #echo $! > tpid
        getPID
        if [ $TPID -ne 0 ]; then
