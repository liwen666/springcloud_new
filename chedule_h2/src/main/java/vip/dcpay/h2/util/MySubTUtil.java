...
//    }
    ...
    public static <T> List<List<T>> subList(List<T> tList, Integer count) {
        int totle = tList.size();
        int num = (totle-1)/count+1;
        List<List<T>> list = new ArrayList<>();
        for(int i=0;i<count;i++){
            if((i+1)*num>tList.size()){
                list.add( tList.subList(i*num,tList.size()));
                continue;
            }
            list.add( tList.subList(i*num,(i+1)*num));
        }
        return list;
