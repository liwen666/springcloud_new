package com.lw.common.config.system;

import com.lw.common.system.GlobalConstant;
...
public class H2Service implements ApplicationRunner, ApplicationListener<ContextClosedEvent> {
    ...
    public void startH2() {
        ...
        try {
            if (GlobalConstant.H2_CONFIG_TCPALLOWOTHERS) {
                server = Server.createTcpServer(new String[]{"-tcpAllowOthers", "-tcpPort", "8043"}).start();
            } else {
                server = Server.createTcpServer(new String[]{"-tcp", "-tcpPort", "8043"}).start();
            }
