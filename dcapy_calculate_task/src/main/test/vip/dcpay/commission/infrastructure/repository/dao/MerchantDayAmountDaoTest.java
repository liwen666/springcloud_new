...
public class MerchantDayAmountDaoTest {
    ...
    @Test
    public void update() {
        MerchantDayAmount merchantDayAmount0 = merchantDayAmountDao.selectOne(Wrappers.<MerchantDayAmount>lambdaQuery().eq(MerchantDayAmount::getCreateTime, "2019-08-14").eq(MerchantDayAmount::getMerchantId, 1));
  if(null == merchantDayAmount0){
      MerchantDayAmount build = MerchantDayAmount.builder().merchantId(1l).createTime(DateUtils.parse("2019-08-14")).remark(1).dayAmountSum(new BigDecimal(100)).build();
      merchantDayAmountDao.insert(build);
